package peraride.ce.pdn.edu.peraride.api.response.factory;

import com.fasterxml.jackson.annotation.JsonProperty;

import peraride.ce.pdn.edu.peraride.api.response.Ancestor;
import peraride.ce.pdn.edu.peraride.data.model.response.DockStationResponseSerializer;
import peraride.ce.pdn.edu.peraride.data.model.response.DockStationSerializer;
import peraride.ce.pdn.edu.peraride.data.model.response.LoginSerializer;

/**
 * Created by user on 5/29/2018.
 */

public class DockStationResponse extends Ancestor<DockStationResponseSerializer> {

    protected DockStationResponse(@JsonProperty("data") DockStationResponseSerializer dataSet) {
        super(dataSet);
    }
}
